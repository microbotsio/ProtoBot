...
bool ProtoBot::Run(uint8_t loop_timer) {
  if (myCodeCell.Run(100)) {
    ...
    if (_send_timer >= _send_thr) {  // Update metrics every 100ms
      ...
      BLE_Update(_mode, 56, ((uint16_t)_yaw));
    }
        ...
        if (BlockSave == 1) {
          ...
          if ((shapeOnOFF) && (reverse_button != 4) && (_init_timer >= 30)) {
            ...
            if (avoidOnOFF) {
              ...
              _send_thr = 10;  // Update metrics every 100ms
              AvoidUpMode();
            } else if ((((_pitch > 225) && (_pitch < 315)) || ((_pitch > 45) && (_pitch < 135))) && ((_roll > 350) || (_roll < 10) || ((_roll < 190) && (_roll > 170)) || (_balance_mode))) {
              _balance_mode = 1;
            ...
            if ((FrontAvoidDetect()) && (_drive_angle != 180) && (_drive_angle != 360) && (_drive_angle != 0)) {  //Proxmity detected
#if defined(ARDUINO_ESP32C6_DEV)
...
bool ProtoBot::FrontAvoidInit() {
  ...
  Wire.write((uint8_t)0x28);  // CONF2: single mode x8, 16-bit output, default sensitivity, interrupts off
  err = Wire.endTransmission(true);
  if (err) {
    Serial.print(">> Error: EyeLight Sensor not found - Check Hardware");
    return false;
  }
  ...
  Wire.write((uint8_t)0x05);  // MS: LED_I bits => 0b101 = 200 mA
  err = Wire.endTransmission(true);
  if (err) {
    Serial.print(">> Error: EyeLight Sensor not found - Check Hardware");
    return false;
...
void ProtoBot::FrontAvoidRead() {
  if (FrontAvoidOnOFF) {
    ...
    float speedm1 = (_m1_speed / speed_level[speed_reduction]);
    float speedm2 = (_m2_speed / speed_level[speed_reduction]);

    myCodeCell.Drive1.Drive(_motor2_dir, 0);
    myCodeCell.Drive2.Drive(_motor1_dir, 0);

    // Register pointer -> repeated start
    Wire.beginTransmission(VCNL4030_ADDRESS);
    Wire.write((uint8_t)0x08);  // PS_DATA
    Wire.endTransmission(false);

    uint8_t got = Wire.requestFrom((uint8_t)VCNL4030_ADDRESS, (uint8_t)2, (bool)true);  // Read 2 bytes
    if (got != 2) {
      while (Wire.available()) (void)Wire.read();
      Serial.println("VCNL4030 read err");
      return;
    }

    uint8_t lo = Wire.read(), hi = Wire.read();

    myCodeCell.Drive1.Drive(_motor2_dir, (uint8_t)speedm1);
    myCodeCell.Drive2.Drive(_motor1_dir, (uint8_t)speedm2);

    _eyelight_proximity = ((uint16_t)hi << 8) | lo;

    if ((_eyelight_proximity == 0) || (_eyelight_proximity == 0xFFFF)) {
      FrontAvoidInit();  //Reset sensor
...
void ProtoBot::FrontPID() {
  _Kp = 0.5;
  _Ki = 0;
  _Kd = 0.002;  //  _Kd = 0.005;

  if (FrontAvoidDetect() || (_Front_Flag)) {
    myEyeLight.show_Static(500, 0XFFFFFF);
    if (!_Front_Flag) {
      _Front_proximity = _eyelight_proximity;  //first time save
      _Front_Flag = 1;
    }
    if (_eyelight_proximity < (_Front_proximity - 5)) {
      //Releasing
      ...
      //Detected
      float _control_front = PID_Controller(_Front_proximity, _eyelight_proximity);
      ...
      if (_control_front >= 0) {
        ...
        _motor1_dir = 0;
        _motor2_dir = 0;
        _control_front = abs(_control_front);
      }
      if (_control_front < 130) {
        _control_front = 0;
      }

      long motor_duty = (long)constrain(_control_front, 0, 100);
      motor_duty = map(motor_duty, 0, 100, (MOTOR_DEADZONE - 10), MOTOR_MAXSPEED);
