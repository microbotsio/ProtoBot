...
bool eyelight_ambient_enable =0, hsv_flag = 0, animation_done = 0, dir_last = 0, animation_flag = 1, connect_flag = 0;
...
void EyeLight::show_Default(uint16_t intervalMs, uint8_t r, uint8_t g, uint8_t b) {
  ...
  for (int t = 0; t < tailLen; t++) {
    int idx = head - t * dir;
    if(idx > (LED_NUM/2)){
      eyelight_ambient_enable =1;
    }
    else{
      eyelight_ambient_enable =0;
    }
    if (idx < 0 || idx >= LED_NUM) break;
    uint8_t fade = map(t, 0, tailLen - 1, 255, 40);
    ProtoBotDisplay.setPixelColor(idx, scaleColor(headColor, fade));
...
const uint32_t warnCore   = ProtoBotDisplay.gamma32(ProtoBotDisplay.Color(0xFF,   0x10,   0)); // vivid red
const uint32_t warnMiddle = ProtoBotDisplay.gamma32(ProtoBotDisplay.Color(0xFF, 0x66,   0)); // bright yellow-orange
