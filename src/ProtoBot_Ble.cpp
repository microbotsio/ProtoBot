...
uint8_t BlockIndex_last=0, BlockIndex = 0, rx_joystick_x = 100, rx_joystick_y = 100, battery_val_last = 0, mode_val_last = 0xFF, reverse_button = 0, first_connect = 0, displayOnOFF = 0, displayRed = 0xff, displayGreen = 0xff, displayBlue = 0xff, touchGesture = 0, speed_reduction = 3;
uint16_t BlockTimer = 0, eepTimer = 0, sleepTimer = 0, heading_val_last = 0, shape_num = 0, shape_thr = 100, shape_timer = 0;
...
void BLE_BlockUpdate(uint8_t mode_val, uint8_t block_index_val) {
  if ((mode_val != mode_val_last) || (block_index_val != BlockIndex_last)) {
    uint8_t modePacket[2] = { mode_val, block_index_val};
    pModeCharacteristic->setValue(modePacket, 2);
    pModeCharacteristic->notify();
  }
  mode_val_last = mode_val;
  BlockIndex_last = block_index_val;
}
...
void BLE_Update(uint8_t mode_val, uint8_t battery_val, uint16_t heading_val) {
  if (isAppConnectionReady) {
    ...
    if ((mode_val != mode_val_last) ) {
      uint8_t modePacket[2] = { mode_val, 0 };
      pModeCharacteristic->setValue(modePacket, 2);
      pModeCharacteristic->notify();
    }
    mode_val_last = mode_val;
