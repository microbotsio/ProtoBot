/*
  Example: ProtoBot Triangle Shape Driving Demo
  Board: ProtoBot (CodeCell C6 Drive inside)

  Overview:
  - Demonstrates how to make ProtoBot drive in a triangle shape
  - Uses the built-in DriveTriangle() function
...
void loop() {
  if (myProtoBot.Run(1)) {  // Run ProtoBot service loop at 1 Hz 
    myProtoBot.DriveTriangle(SPEED_FAST, 70, SHAPE_LOOP);  // Drive in a circle shape with 70% shape size
  }
}